...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
    <div>
        <h1 class="text-[#0d131c] text-3xl md:text-4xl font-bold">Gestión de Especialidades</h1>
        <p class="text-[#49699c] text-base mt-1">Administra las especialidades médicas disponibles.</p>
    </div>
    <div class="mt-4 md:mt-0">
        <a href="{{ route('admin.especialidad.create') }}" class="btn-primary inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Total Especialidades</p>
                <p class="text-[#0d131c] text-2xl font-bold">{{ $especialidades->count() }}</p>
            </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Doctores Asignados</p>
    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Citas por Especialidad</p>
...
<div class="bg-white rounded-xl shadow-md overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-[#0d131c] text-xl font-semibold">Lista de Especialidades</h2>
    </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="text-left py-4 px-6 text-sm font-medium text-[#49699c]">Nombre</th>
                    <th class="text-left py-4 px-6 text-sm font-medium text-[#49699c]">Doctores</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                                <span class="text-blue-600 font-semibold text-sm">
                                    {{ strtoupper(substr($especialidad->name ?? 'E', 0, 1)) }}
                                </span>
                            </div>
                            <div>
                                <p class="text-[#0d131c] font-medium">{{ $especialidad->name }}</p>
                                <p class="text-[#49699c] text-sm">Especialidad Médica</p>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-6">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            {{ $especialidad->doctors->count() }} doctores
                        </span>
                    </td>
                    <td class="py-4 px-6">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-2">
                            ...
                            <form id="form-{{ $especialidad->id }}" action="#" method="POST" class="hidden">
                            @csrf
                            @method('DELETE')
                        </form>
                        </div>
                ...
                <tr>
                    <td colspan="5" class="py-12 text-center">
                        ...
                        <p class="text-[#49699c] text-lg font-medium">No hay especialidades registradas</p>
                        <p class="text-[#49699c] text-sm mt-1">Comienza agregando la primera especialidad al sistema</p>
                    </td>
                </tr>
                @endforelse
        </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('btn-delete')) {
            ...
            confirmMessage.textContent = '¿Está seguro que desea eliminar esta especialidad? Esta acción no se puede deshacer.';
