...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
    <div>
        <h1 class="text-[#0d131c] text-3xl md:text-4xl font-bold">Gestión de Secretarias</h1>
        <p class="text-[#49699c] text-base mt-1">Administra el personal administrativo del sistema.</p>
    </div>
    <div class="mt-4 md:mt-0">
        <a href="{{ route('admin.secretaria.create') }}" class="btn-primary inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Total Secretarias</p>
                <p class="text-[#0d131c] text-2xl font-bold">{{ $secretarias->total() }}</p>
            </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Activas</p>
                <p class="text-[#0d131c] text-2xl font-bold">{{ $secretarias->where('status', 'active')->count() }}</p>
            </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#49699c] text-sm font-medium">Citas Gestionadas</p>
...
<div class="bg-white rounded-xl shadow-md overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-[#0d131c] text-xl font-semibold">Lista de Secretarias</h2>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($secretarias as $index => $secretaria)
                <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full bg-pink-100 flex items-center justify-center mr-3">
                                <span class="text-pink-600 font-semibold text-sm">
                                    {{ strtoupper(substr($secretaria->user->profile->first_name ?? 'S', 0, 1)) }}
                                </span>
                            </div>
                            <div>
                                <p class="text-[#0d131c] font-medium">{{ $secretaria->user->profile->first_name }} {{ $secretaria->user->profile->last_name }}</p>
                                <p class="text-[#49699c] text-sm">Secretaria</p>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-6 text-sm text-[#0d131c]">
                        {{ $secretaria->user->email }}
                    </td>
                    <td class="py-4 px-6 text-sm text-[#0d131c]">
                        {{ $secretaria->user->profile->phone }}
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-2">
                            ...
                            <form id="form-{{ $secretaria->id }}" action="{{ route('admin.secretaria.destroy', $secretaria->id) }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
                ...
                <tr>
                    <td colspan="6" class="py-12 text-center">
                        ...
                        <p class="text-[#49699c] text-lg font-medium">No hay secretarias registradas</p>
                        <p class="text-[#49699c] text-sm mt-1">Comienza agregando la primera secretaria al sistema</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="px-6 py-4 border-t border-gray-200">
        <div class="flex items-center justify-between">
            <div class="text-sm text-[#49699c]">
                Mostrando {{ $secretarias->firstItem() }} a {{ $secretarias->lastItem() }} de {{ $secretarias->total() }} resultados
            </div>
            <div class="flex items-center gap-2">
                @if($secretarias->onFirstPage())
                    <span class="px-3 py-2 text-sm text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">Anterior</span>
                @else
                    <a href="{{ $secretarias->previousPageUrl() }}" class="px-3 py-2 text-sm text-[#0d131c] bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Anterior</a>
                @endif
                
                @foreach($secretarias->getUrlRange(1, $secretarias->lastPage()) as $page => $url)
                    @if($page == $secretarias->currentPage())
                ...
                @if($secretarias->hasMorePages())
                    <a href="{{ $secretarias->nextPageUrl() }}" class="px-3 py-2 text-sm text-[#0d131c] bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Siguiente</a>
                @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('btn-delete')) {
            ...
            confirmMessage.textContent = '¿Está seguro que desea eliminar esta secretaria? Esta acción no se puede deshacer.';
