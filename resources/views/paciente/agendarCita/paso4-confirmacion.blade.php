...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('paciente.agendarCita.store') }}" method="POST" enctype="multipart/form-data" id="confirmacionForm">
        @csrf
        ...
        <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-8 text-sm">
                <div>
                    ...
                    <p class="font-medium text-gray-700">
                        {{ $paciente->user->profile->first_name ?? 'Paciente' }} {{ $paciente->user->profile->last_name ?? '' }}
                    </p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="font-medium text-gray-700">
                        Dr. {{ $medico->user->profile->first_name ?? 'Médico' }} {{ $medico->user->profile->last_name ?? '' }}
                    </p>
                </div>
                <div>
            ...
            <div class="space-y-4 mb-8">
                <div>
                    <input class="sr-only" id="onlinePayment" name="payment_method" onchange="togglePaymentFields()" type="radio" value="online" />
                    <label class="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 transition-colors duration-150" for="onlinePayment">
                        <span class="material-icons mr-3 text-blue-600">credit_card</span>
                        <span class="font-medium text-gray-700">Pago en Línea</span>
                    </label>
                </div>
                <div>
                    <input class="sr-only" id="transferPayment" name="payment_method" onchange="togglePaymentFields()" type="radio" value="transfer" />
                    <label class="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 transition-colors duration-150" for="transferPayment">
                ...
                <div>
                    <input class="sr-only" id="clinicPayment" name="payment_method" onchange="togglePaymentFields()" type="radio" value="clinic" />
                    <label class="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 transition-colors duration-150" for="clinicPayment">
                    ...
                    <div class="flex">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-yellow-700 mt-1">
                                Su cita será reservada. Deberá realizar el pago directamente en la clínica antes de su consulta.
                            </p>
                        </div>
                    </div>
                </div>
